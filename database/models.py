...
def get_user(user_id): return db.fetchone("SELECT * FROM users WHERE user_id=?", (user_id,))
def create_user(user_id, username, first_name):
    db.insert('users', {'user_id': user_id, 'username': username or '', 'first_name': first_name or '', 'joined_date': datetime.now().isoformat()})
def get_pending_payments(): return db.fetchall("SELECT * FROM payments WHERE status='pending' ORDER BY created_at DESC")
...
def get_total_stats():
    ...
    return {'users': users['c'] if users else 0, 'paid': paid['c'] if paid else 0, 'pays': pays['c'] if pays else 0, 'revenue': revenue['c'] or 0 if revenue else 0}
